...
export default function RedesignedPrintingScreen() {
  ...
  return (
    <SafeAreaView style={[styles.container, { height: insets.top }]}>
      {/* Header */}
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        {step === 1 && (
          <>
            <SectionTitle
              ...
              subtitle="Find the nearest available printer"
            />
            {PRINTERS.map((printer) => (
              <TouchableOpacity
                ...
                style={[
                  styles.printerCard,
                  selectedPrinter?.id === printer.id && styles.selectedCard,
                ]}
                onPress={() => setSelectedPrinter(printer)}
              >
                <Image
                  source={{ uri: printer.image }}
                  style={styles.printerImg}
                />
                <View style={styles.printerInfo}>
                  <Text style={styles.printerName}>{printer.name}</Text>
                  <Text style={styles.printerLoc}>
                    <Ionicons name="location" size={12} /> {printer.location}
                  </Text>
                  <View style={styles.tagRow}>
                    <View style={styles.tag}>
                      <Text style={styles.tagText}>₵{printer.price}/pg</Text>
                    </View>
                    <View style={styles.tag}>
                      <Text style={styles.tagText}>{printer.distance}</Text>
                    </View>
                  </View>
                </View>
              </TouchableOpacity>
        ...
        {step === 2 && (
          <>
            <SectionTitle
              title="Configure Job"
              subtitle="Upload PDF and choose settings"
            />
            ...
            <View style={styles.configBox}>
              ...
              <View style={styles.toggleRow}>
                <TouchableOpacity
                  style={[
                    styles.toggleBtn,
                    colorMode === "bw" && styles.toggleActive,
                  ]}
                ...
                <TouchableOpacity
                  style={[
                    styles.toggleBtn,
                    colorMode === "color" && styles.toggleActive,
                  ]}
          ...
          <View style={styles.ticketCard}>
            ...
            <View style={styles.ticketBody}>
              ...
              <Text style={styles.ticketInstructions}>
                Show this to the attendant at {selectedPrinter.name}
              </Text>
              <TouchableOpacity
      ...
      {step < 3 && (
        <View style={[styles.footer, { paddingBottom: 60 + insets.bottom}]}>
          ...
          <TouchableOpacity
            style={[
              styles.mainBtn,
              (!selectedPrinter || (step === 2 && documents.length === 0)) &&
                styles.disabledBtn,
            ]}
          ...
          >
            {isProcessing ? (
              ...
              <Text style={styles.mainBtnText}>
                {step === 1 ? "Configure Print" : "Pay via Mobile Money"}
              </Text>
            )}
          </TouchableOpacity>
        </View>
